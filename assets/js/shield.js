document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderPosts() {
    ...
    if (filteredPosts.length === 0) {
      wrapper.innerHTML = `<p style="text-align:center; padding:40px; color:#666; font-size:18px; grid-column:1/-1;">No safeguards found. Please try another search query.</p>`;
      return;
    }
    ...
    chunk.forEach(p => {
      // Escape title to prevent HTML breakage inside attributes
        ...
        <div class="blog-card js-post" data-lang="${p.lang || 'en'}" data-post-url="${p.url}">
          <a href="${p.url}" style="text-decoration: none; color: inherit; display: block;">
            <div class="blog-card__thumb">
              <img src="${p.image || '/assets/images/default-thumb.png'}" alt="${escapedTitle} - TyagiHub" title="${escapedTitle} - TyagiHub" loading="lazy" width="354" height="236" class="shield-card-img">
            </div>
